...
class AnchorRewardsTest (DefiTestFramework):
    def set_test_params(self):
        self.num_nodes = 3
        self.extra_args = [
            [ "-dummypos=1", "-spv=1", "-fakespv=1", '-amkheight=0', "-dakotaheight=0"],
            [ "-dummypos=1", "-spv=1", "-fakespv=1", '-amkheight=0', "-dakotaheight=0"],
            [ "-dummypos=1", "-spv=1", "-fakespv=1", '-amkheight=0', "-dakotaheight=0"],
        ]
    ...
    def run_test(self):
        ...
        # Get some data, Travis seems to fail on next wait_until
        ...
        assert_equal(self.nodes[0].spv_listanchorauths()[0]['blockHeight'], 15)
